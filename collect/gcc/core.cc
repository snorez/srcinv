/*
 * collect GCC compiling program info compiled by GCC at ALL_IPA_PASSES_END
...
static int get_compiling_args(int fmt)
{
	/*
	 * UPDATE:
...
static void do_specific_language_function(void *node, int flag);
...
static void do_function(struct function *node, int flag)
{
	...
	do_specific_language_function(node0->language, 1);
	do_tree(node0->decl);
...
static void do_tree(tree node)
{
	...
	case tcc_declaration:
	{
		...
		case TEMPLATE_DECL:
			return do_template_decl(node, 1);
		case USING_DECL:
			return do_using_decl(node, 1);
		default:
	...
	case tcc_exceptional:
	{
		...
		case TEMPLATE_PARM_INDEX:
			return do_template_parm_index(node, 1);
		case TEMPLATE_INFO:
			return do_template_info(node, 1);
		case OVERLOAD:
			return do_overload(node, 1);
		case DEFERRED_NOEXCEPT:
			return do_deferred_noexcept(node, 1);
		case BASELINK:
			return do_baselink(node, 1);
		case STATIC_ASSERT:
			return do_static_assert(node, 1);
		case TRAIT_EXPR:
			return do_trait_expr(node, 1);
		default:
			fprintf(stderr, "missing %s\n", tree_code_name[code]);
			BUG();
		}
...
static void do_identifier(tree node, int flag)
{
	return do_specific_identifier(node, flag);
}
...
static void do_decl_common(tree node, int flag)
{
	...
	do_tree(node0->attributes);
	do_tree(node0->abstract_origin);
	do_specific_lang_decl((void *)node0->lang_specific, 1);
}
...
static void ast_collect(void *gcc_data, void *user_data)
{
	 ...
	 *
	 * XXX: only collect the global data if obj_idx is not empty.
	 */
	if (obj_idx && (!global_collected)) {
		collect_global_data();
		global_collected = 1;
	}
