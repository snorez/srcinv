...
gen_file()
{
	...
	CONTENT+=$'SELF_CFLAGS+=-DCONFIG_BITS_PER_UNIT=8\n'
	CONTENT+=$'#SELF_CFLAGS+=-DUSE_NCURSES\n'
	CONTENT+=$'#SELF_CFLAGS+=-DCONFIG_GUESS_DSV_RANDOM\n'
	CONTENT+=$'\n'
